...
export default class GradientCircleShaderType extends RectShaderType {
	...
	render(gl: WebGLRenderingContext, options: Record<string, any>): void {
		...
		// Adds color -- added by me
		let color = gl.getUniformLocation(program, "asteroid_color");
		var color_choice = options.color.toWebGL();
		gl.uniform4f(color, color_choice[0], color_choice[1], color_choice[2], color_choice[3]);
