...
export default class Homework1_Scene extends Scene {
	...
	private MIN_SPAWN_DISTANCE: number = 100; //this is the minimum distance that object can spawn next to each other
	...
	*/
	...
	spawnAsteroid(): void {
		...
		for(index = 0; index< this.asteroids.length; index++){
			if(!this.asteroids[index].visible){
				// We found a dead asteroid
				asteroid = this.asteroids[index];
				break;
			}
		}
		...
		if(asteroid !== null){
			...
			//changes color -- added by me
			var color_choice = [Color.CYAN, Color.MAGENTA, Color.ORANGE, Color.WHITE, Color.BLUE, Color.RED];
			let num = Math.floor(Math.random() * (6));
			this.asteroids[index].setColor(color_choice[num]);
			...
			// Loop on position until we're clear of the player
			asteroid.position = RandUtils.randVec(0, viewportSize.x, 0, viewportSize.y); //creates a random vector
	...
	handleScreenWrap(node: GameNode, viewportCenter: Vec2, paddedViewportSize: Vec2): void {
		...
		var half_x = paddedViewportSize.x / 2.0;
		var half_y = paddedViewportSize.y / 2.0;
		var left_edge = viewportCenter.x - half_x;
		var right_edge = viewportCenter.x + half_x;
		var top_edge = viewportCenter.y - half_y;
		var bottom_edge = viewportCenter.y + half_y;

		if(current_position.x < (left_edge - 1)){
			node.positionX = right_edge;
		}else if(current_position.x > (right_edge + 1)){
			node.positionX = left_edge;
		}else if(current_position.y < (top_edge - 1)){
			node.positionY = bottom_edge;
		}else if(current_position.y > (bottom_edge + 1)){
			node.positionY = top_edge;
		}
	}
